...
public class EnemyBullet extends GameObj implements Hostile {
	...
	public EnemyBullet(int x, int y, int speed, Game game, Controller c,
			Sprites s) {
		super(x, y);
		this.game = game;
		this.speed = speed;
	...
	public void update() {
		...
		if (!hit) {
			...
			if (Physics.collision(game.getPlane(), this)) {
				hit = true;
				boom.play(false);
				game.getPlane().setHEALTH(game.getPlane().getHEALTH() - 15);
			}

			
			if (Physics.collision(game.getPlane2(), this) && game.getState() == game.getState().MULTI) {
				hit = true;
				boom.play(false);
				game.getPlane2().setHEALTH(game.getPlane2().getHEALTH() - 15);	
			}
			anim.runAnimation();
		} else if (hit) {
			if (timer > 40) {
